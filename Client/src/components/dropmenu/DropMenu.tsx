...
interface Props {
  ...
  dir: 'left' | 'right',
  width: number,
  hasPrintWindow: boolean
}

export const DropMenu: React.FC<Props> = ({ menuOp, dir, width, hasPrintWindow }): JSX.Element => {
  const { option } = useContext(NavigationContext)

  const tableName = LINKS[option - 1]
  const docName = tableName.toLowerCase().split(' ').join('-')
  ...
  return (
    <div className='drop-menu' style={ directionStyle } onClick={ () => hasPrintWindow && printData() }>
      <ul>
