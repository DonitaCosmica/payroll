...
export const useGeneratePrintPage = ({ titlebar, tableId, filterReport, hasForm }: Props): { [key: string]: string } => {
  ...
  const contentHtmlTemplate = !hasForm ? styleHtmlTemplate.replace('<body style="position: relative; height: 100vh">', `
    <body style="position: relative; height: 98vh">
      ${ titlebar }
      <section id="content">
        ${ modifiedDataToPrint }
      </section>
  `) : styleHtmlTemplate.replace('<body style="position: relative; height: 100vh">', `
    <body style="position: relative; height: 98vh">
      ${ titlebar }
      <section id="content">
          ${ modifiedDataToPrint }
